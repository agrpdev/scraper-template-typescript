...
export class BaseScraper {
  ...
  constructor({ customId, apiKey }: { customId: string; apiKey: string }) {
  ...
  async init() {
    const response = await this.fetch({
      ...
      errorMessage: 'Failed to register scraper',
    })
    const data = await response.json()
    console.log('Scraper register:', data)
    if (response.status !== 200) return false

    this.details = { ...data.scraper }
    ...
    if (!data.configured) {
      console.log('Scraper is not configured')
      return false
    }
    return true
  }
